...
class ExtractItems:
    def __init__(
        ...
        self.items_list = file_constants.ITEM_LIST_10_Q
    ...
    @staticmethod
    def remove_html_tables(doc_10q, is_html):
        """
        ...
        :param doc_10q: The 10-q html
        :param is_html: Whether the document contains html code or just plain text
        :return: doc_10q: The 10-q html without numerical tables
        """

        if is_html:
            tables = doc_10q.find_all('table')
            ...
            doc_10q = re.sub(r'<TABLE>.*?</TABLE>', '', str(doc_10q), flags=regex_flags)

        return doc_10q

    def parse_item(self, text, item_index, next_item_list, positions):
        """
        Parses Item N for a 10-q text

        :param text: The 10-q text
        :param item_index: Number of the requested Item/Section of the 10-q text
        :param next_item_list: List of possible next 10-q item sections
        :param positions: List of the end positions of previous item sections
        :return: item_section_10q: The item/section as a text string
        """
        ...
        item_section_10q, positions = ExtractItems.get_item_section_10q(possible_sections_list, text, positions)

        # If item is the last one (usual case when dealing with EDGAR's old .txt files), get all the text from its beginning until EOF.
        if positions:
            if item_index in self.items_list and item_section_10q == '':
                item_section_10q = ExtractItems.get_last_item_section_10q(item_index, text, positions)
            elif item_index == '15':  # Item 15 is the last one, get all the text from its beginning until EOF
                item_section_10q = ExtractItems.get_last_item_section_10q(item_index, text, positions)

        return item_section_10q.strip(), positions

    @staticmethod
    def get_item_section_10q(possible_sections_list, text, positions):
        """
        ...
        if max_match:
            if positions:
                if max_match_offset + max_match.start() >= positions[-1]:
                    item_section_10q = text[max_match_offset + max_match.start(): max_match_offset + max_match.regs[1][0]]
            else:
                item_section_10q = text[max_match_offset + max_match.start(): max_match_offset + max_match.regs[1][0]]
            positions.append(max_match_offset + max_match.end() - len(max_match[1]) - 1)

        return item_section_10q, positions

    @staticmethod
    def get_last_item_section_10q(item_index, text, positions):
        """
        ...
        :param item_index: The index of the item/section in the 10-q ('14' or '15')
        :param text: The whole 10-q text
        :param positions: List of the end positions of previous item sections
        ...
        for item in item_list:
            if item.start() >= positions[-1]:
                item_section_10q = text[item.start():].strip()
                break

        return item_section_10q

    def extract_items(self, filing_metadata):
        """
        Extracts all items/sections for a 10-q file and writes it to a CIK_10q_YEAR.json file (eg. 1384400_10q_2017.json)

        :param filing_metadata: a pandas series containing all filings metadata
        """
        
        absolute_10q_filename = os.path.join(self.raw_files_folder, filing_metadata['filename'])

        with open(absolute_10q_filename, 'r', errors='backslashreplace') as file:
            content = file.read()

        # Remove all embedded pdfs that might be seen in few old 10-q txt annual reports
        ...
        for doc in documents:
            ...
            if doc_type.startswith('10'):
                ...
                is_html = (True if doc_10q.find('td') else False) and (True if doc_10q.find('tr') else False)
                if not is_html:
                    doc_10q = doc
                found_10q = True
                break
        ...
        if not found_10q:
            if documents:
                LOGGER.info(f'Could not find document type 10q for {filing_metadata["filename"]}')
        ...
        for i, item_index in enumerate(self.items_list):
            ...
            item_section_10q, positions = self.parse_item(text, item_index, next_item_list, positions)
            item_section_10q = ExtractItems.remove_multiple_lines(item_section_10q)

            if item_index in self.items_to_extract:
                if item_section_10q != '':
                    all_items_null = False
                json_content[f'item_{item_index}'] = item_section_10q

        if all_items_null:
            LOGGER.info(f'Could not extract any item for {absolute_10q_filename}')
            return None
...
def main():
    """
    Gets the list of 10q files and extracts all textual items/sections by calling the extract_items() function.
    """
