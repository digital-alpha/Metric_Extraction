...
class ExtractItems:
    def __init__(
        ...
        self.items_list = file_constants.ITEM_LIST_8_K
    ...
    @staticmethod
    def remove_html_tables(doc_8k, is_html):
        """
        ...
        :param doc_8k: The 8-k html
        :param is_html: Whether the document contains html code or just plain text
        :return: doc_8k: The 8-k html without numerical tables
        """

        if is_html:
            tables = doc_8k.find_all('table')
            ...
            doc_8k = re.sub(r'<TABLE>.*?</TABLE>', '', str(doc_8k), flags=regex_flags)

        return doc_8k

    def parse_item(self, text, item_index, next_item_list, positions):
        """
        Parses Item N for a 8-k text

        :param text: The 8-k text
        :param item_index: Number of the requested Item/Section of the 8-k text
        :param next_item_list: List of possible next 8-k item sections
        :param positions: List of the end positions of previous item sections
        :return: item_section_8k: The item/section as a text string
        """
        ...
        item_section_8k, positions = ExtractItems.get_item_section_8k(possible_sections_list, text, positions)

        # If item is the last one (usual case when dealing with EDGAR's old .txt files), get all the text from its beginning until EOF.
        if positions:
            if item_index in self.items_list and item_section_8k == '':
                item_section_8k = ExtractItems.get_last_item_section_8k(item_index, text, positions)
            elif item_index == '15':  # Item 15 is the last one, get all the text from its beginning until EOF
                item_section_8k = ExtractItems.get_last_item_section_8k(item_index, text, positions)

        return item_section_8k.strip(), positions

    @staticmethod
    def get_item_section_8k(possible_sections_list, text, positions):
        """
        ...
        if max_match:
            if positions:
                if max_match_offset + max_match.start() >= positions[-1]:
                    item_section_8k = text[max_match_offset + max_match.start(): max_match_offset + max_match.regs[1][0]]
            else:
                item_section_8k = text[max_match_offset + max_match.start(): max_match_offset + max_match.regs[1][0]]
            positions.append(max_match_offset + max_match.end() - len(max_match[1]) - 1)

        return item_section_8k, positions

    @staticmethod
    def get_last_item_section_8k(item_index, text, positions):
        """
        ...
        :param item_index: The index of the item/section in the 8-k ('14' or '15')
        :param text: The whole 8-k text
        :param positions: List of the end positions of previous item sections
        ...
        for item in item_list:
            if item.start() >= positions[-1]:
                item_section_8k = text[item.start():].strip()
                break

        return item_section_8k

    def extract_items(self, filing_metadata):
        """
        Extracts all items/sections for a 8-K file and writes it to a CIK_8k_YEAR.json file (eg. 1384400_8k_2017.json)

        :param filing_metadata: a pandas series containing all filings metadata
        """
        
        absolute_8k_filename = os.path.join(self.raw_files_folder, filing_metadata['filename'])

        with open(absolute_8k_filename, 'r', errors='backslashreplace') as file:
            content = file.read()

        # Remove all embedded pdfs that might be seen in few old 8-k txt annual reports
        content = re.sub(r'<PDF>.*?</PDF>', '', content, flags=regex_flags)

        documents = re.findall('<DOCUMENT>.*?</DOCUMENT>', content, flags=regex_flags)

        doc_8k = None
        found_8k, is_html = False, False
        for doc in documents:
            doc_type = re.search(r'\n[^\S\r\n]*<TYPE>(.*?)\n', doc, flags=regex_flags)
            doc_type = doc_type.group(1) if doc_type else None
            if doc_type.startswith('8'):
                doc_8k = BeautifulSoup(doc, 'lxml')
                is_html = (True if doc_8k.find('td') else False) and (True if doc_8k.find('tr') else False)
                if not is_html:
                    doc_8k = doc
                found_8k = True
                break
        ...
        if not found_8k:
            if documents:
                LOGGER.info(f'Could not find document type 8k for {filing_metadata["filename"]}')
            doc_8k = BeautifulSoup(content, 'lxml')
            is_html = (True if doc_8k.find('td') else False) and (True if doc_8k.find('tr') else False)
            if not is_html:
                doc_8k = content
        ...
        if self.remove_tables:
            doc_8k = self.remove_html_tables(doc_8k, is_html=is_html)
        ...
        text = ExtractItems.strip_html(str(doc_8k))
        ...
        for i, item_index in enumerate(self.items_list):
            ...
            item_section_8k, positions = self.parse_item(text, item_index, next_item_list, positions)
            item_section_8k = ExtractItems.remove_multiple_lines(item_section_8k)

            if item_index in self.items_to_extract:
                if item_section_8k != '':
                    all_items_null = False
                json_content[f'item_{item_index}'] = item_section_8k

        if all_items_null:
            LOGGER.info(f'Could not extract any item for {absolute_8k_filename}')
            return None
...
def main():
    """
    Gets the list of 8k files and extracts all textual items/sections by calling the extract_items() function.
    """

    with open('config.json') as fin:
        config = json.load(fin)['extract_items_8k']
